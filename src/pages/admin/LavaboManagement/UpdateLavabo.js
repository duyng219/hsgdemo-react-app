...
import { getOneLavabo, updateLavabo } from "../../../redux/slices/lavaboSlice";
...
const UpdateLavabo = () => {
  ...
  const { id } = useParams();
  const { lavabo } = useSelector((state) => state.lavabo);
  ...
  const handleUpload = async (e) => {
    ...
    setUrlImage(url);
    if (!url) return;
    const storageRef = ref(storage, `/hoasen/images/lavabo/${file.name}`);
  ...
  useEffect(() => {
    dispatch(getOneLavabo(id));
    setInputValue({
      name: lavabo.name,
      price: lavabo.price,
      image: lavabo.image,
    });
    setUrlImage(lavabo.image);
  }, [dispatch, id, lavabo.name, lavabo.price, lavabo.image]);
      ...
      <div className="container-login100">
        <div className="wrap-login100 p-l-55 p-r-55 p-t-65 p-b-54">
          <form onSubmit={handleUpdate} className="login100-form validate-form">
            <span className="login100-form-title p-b-30">Update lavabo</span>
            <div
