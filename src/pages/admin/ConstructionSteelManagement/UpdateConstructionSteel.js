...
import {
  getOneConstructionSteel,
  updateConstructionSteel,
} from "../../../redux/slices/constructionSteelSlice";
import { getDownloadURL, ref, uploadBytesResumable } from "firebase/storage";
import { storage } from "../../../firebase/firebaseConnect";
import { notiFunc } from "../../../utils/settings/notification/notiFunc";

const UpdateConstructionSteel = () => {
  const [urlImage, setUrlImage] = useState("");
  const { id } = useParams();
  const { constructionSteel } = useSelector((state) => state.constructionSteel);
  const dispatch = useDispatch();

  const [inputValue, setInputValue] = useState({
    name: constructionSteel.name,
    price: constructionSteel.price,
    image: urlImage,
  });
  ...
  const handleUpload = async (e) => {
    ...
    const storageRef = ref(
      storage,
      `/hoasen/images/constructionSteel/${file.name}`
    );
  ...
  useEffect(() => {
    dispatch(getOneConstructionSteel(id));
    setInputValue({
      name: constructionSteel.name,
      price: constructionSteel.price,
      image: constructionSteel.image,
    });
    setUrlImage(constructionSteel.image);
  }, [
    dispatch,
    id,
    constructionSteel.name,
    constructionSteel.price,
    constructionSteel.image,
  ]);
...
export default UpdateConstructionSteel;
