...
const UpdateLedLight = () => {
  ...
  const handleUpload = async (e) => {
    ...
    setUrlImage(url);
    if (!url) return;
    const storageRef = ref(storage, `/hoasen/images/ledLight/${file.name}`);
  ...
  useEffect(() => {
    dispatch(getOneLedLight(id));
    setInputValue({
      name: ledLight.name,
      price: ledLight.price,
      image: ledLight.image,
    });
    setUrlImage(ledLight.image);
  }, [dispatch, id, ledLight.name, ledLight.price, ledLight.image]);
      ...
      <div className="container-login100">
        <div className="wrap-login100 p-l-55 p-r-55 p-t-65 p-b-54">
          <form onSubmit={handleUpdate} className="login100-form validate-form">
            <span className="login100-form-title p-b-30">Update ledLight</span>
            <div
