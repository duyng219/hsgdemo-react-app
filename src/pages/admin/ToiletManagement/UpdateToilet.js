...
import { getOneToilet, updateToilet } from "../../../redux/slices/toiletSlice";
...
const UpdateToilet = () => {
  ...
  const handleUpload = async (e) => {
    ...
    setUrlImage(url);
    if (!url) return;
    const storageRef = ref(storage, `/hoasen/images/Toilet/${file.name}`);
  ...
  useEffect(() => {
    dispatch(getOneToilet(id));
    setInputValue({
      name: toilet.name,
      price: toilet.price,
      image: toilet.image,
    });
    setUrlImage(toilet.image);
  }, [dispatch, id, toilet.name, toilet.price, toilet.image]);
      ...
      <div className="container-login100">
        <div className="wrap-login100 p-l-55 p-r-55 p-t-65 p-b-54">
          <form onSubmit={handleUpdate} className="login100-form validate-form">
            <span className="login100-form-title p-b-30">Update toilet</span>
            <div
