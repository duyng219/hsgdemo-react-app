...
import {
  getOnePlasticPipe,
  updatePlasticPipe,
} from "../../../redux/slices/plasticPipeSlice";
import { getDownloadURL, ref, uploadBytesResumable } from "firebase/storage";
import { storage } from "../../../firebase/firebaseConnect";
import { notiFunc } from "../../../utils/settings/notification/notiFunc";

const UpdatePlasticPipe = () => {
  const [urlImage, setUrlImage] = useState("");
  const { id } = useParams();
  const { plasticPipe } = useSelector((state) => state.plasticPipe);
  const dispatch = useDispatch();

  const [inputValue, setInputValue] = useState({
    name: plasticPipe.name,
    price: plasticPipe.price,
    image: urlImage,
  });
  ...
  const handleUpload = async (e) => {
    ...
    setUrlImage(url);
    if (!url) return;
    const storageRef = ref(storage, `/hoasen/images/plasticPipe/${file.name}`);
  ...
  useEffect(() => {
    dispatch(getOnePlasticPipe(id));
    setInputValue({
      name: plasticPipe.name,
      price: plasticPipe.price,
      image: plasticPipe.image,
    });
    setUrlImage(plasticPipe.image);
  }, [dispatch, id, plasticPipe.name, plasticPipe.price, plasticPipe.image]);
...
export default UpdatePlasticPipe;
