...
import { getOneDoor, updateDoor } from "../../../redux/slices/doorSlice";
...
const UpdateDoor = () => {
  ...
  const handleUpload = async (e) => {
    ...
    setUrlImage(url);
    if (!url) return;
    const storageRef = ref(storage, `/hoasen/images/door/${file.name}`);
  ...
  useEffect(() => {
    dispatch(getOneDoor(id));
    setInputValue({
      name: door.name,
      price: door.price,
      image: door.image,
    });
    setUrlImage(door.image);
  }, [dispatch, id, door.name, door.price, door.image]);
      ...
      <div className="container-login100">
        <div className="wrap-login100 p-l-55 p-r-55 p-t-65 p-b-54">
          <form onSubmit={handleUpdate} className="login100-form validate-form">
            <span className="login100-form-title p-b-30">Update door</span>
            <div
