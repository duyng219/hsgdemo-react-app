...
import { getOnePurlin, updatePurlin } from "../../../redux/slices/purlinSlice";
...
const UpdatePurlin = () => {
  ...
  const { id } = useParams();
  const { purlin } = useSelector((state) => state.purlin);
  ...
  const handleUpload = async (e) => {
    ...
    setUrlImage(url);
    if (!url) return;
    const storageRef = ref(storage, `/hoasen/images/purlin/${file.name}`);
  ...
  useEffect(() => {
    dispatch(getOnePurlin(id));
    setInputValue({
      name: purlin.name,
      price: purlin.price,
      image: purlin.image,
    });
    setUrlImage(purlin.image);
  }, [dispatch, id, purlin.name, purlin.price, purlin.image]);
      ...
      <div className="container-login100">
        <div className="wrap-login100 p-l-55 p-r-55 p-t-65 p-b-54">
          <form onSubmit={handleUpdate} className="login100-form validate-form">
            <span className="login100-form-title p-b-30">Update purlin</span>
            <div
