...
import {
  getOneFloorBrick,
  updateFloorBrick,
} from "../../../redux/slices/floorBrickSlice";
import { getDownloadURL, ref, uploadBytesResumable } from "firebase/storage";
import { storage } from "../../../firebase/firebaseConnect";
import { notiFunc } from "../../../utils/settings/notification/notiFunc";

const UpdateFloorBrick = () => {
  const [urlImage, setUrlImage] = useState("");
  const { id } = useParams();
  const { floorBrick } = useSelector((state) => state.floorBrick);
  const dispatch = useDispatch();

  const [inputValue, setInputValue] = useState({
    name: floorBrick.name,
    price: floorBrick.price,
    image: urlImage,
  });
  ...
  const handleUpload = async (e) => {
    ...
    setUrlImage(url);
    if (!url) return;
    const storageRef = ref(storage, `/hoasen/images/floorBrick/${file.name}`);
  ...
  useEffect(() => {
    dispatch(getOneFloorBrick(id));
    setInputValue({
      name: floorBrick.name,
      price: floorBrick.price,
      image: floorBrick.image,
    });
    setUrlImage(floorBrick.image);
  }, [dispatch, id, floorBrick.name, floorBrick.price, floorBrick.image]);
...
export default UpdateFloorBrick;
